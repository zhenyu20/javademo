...
- 4 ，用加粗斜体标识，nums2 = [1,2,3,4]。不存在下一个更大元素，所以答案是 -1 。
 ...
 Related Topics 栈 数组 哈希表 单调栈 👍 1103 👎 0
...
public class I496NextGreaterElementI{
    public static void main(String[] args) {
        Solution solution = new I496NextGreaterElementI().new Solution();
...
class Solution {
    public int[] nextGreaterElement(int[] nums1, int[] nums2) {
        ...
        for (int num : nums2) {
            while (!stack.isEmpty() && (top = stack.peek()) < num) {
                stack.pop();
                map.put(top, num);
            }
            stack.push(num);
